...
bool jr2_work_decode(const json_t *val, struct work *work) { return rpc2_job_decode(val, work); }
...
*/
    ...
    while (1) {
        ...
        if (nonce_found && !opt_benchmark) {  // 4 way with multiple nonces, copy individually to work and submit.
            if (nonce_found > 1)
            ...
            else {  // only 1 nonce, in work ready to submit.
...
typedef struct pool_connection_data_ {
    char *url;
    char *user;
    char *passwd;
...
int main(int argc, char *argv[]) {
    ...
    if (use_gpu != NULL && opt_algo != ALGO_ARGON2D4096 && opt_algo != ALGO_ARGON2D500 && opt_algo != ALGO_ARGON2D250 && opt_algo != ALGO_ARGON2D16000) {
        fprintf(stderr, "%s: GPU can only be used with argon2i and argon2d algos.\n", argv[0]);
        show_usage_and_exit(1);
    }

    if (use_gpu != NULL && !want_stratum) {
        fprintf(stderr, "%s: GPU can only be used with stratum protocol.\n", argv[0]);
        show_usage_and_exit(1);
    }
...
*/
    ...
    for (i = 0; i < opt_n_threads; i++) {
        if (!algo_gate.miner_thread_pre_init(i)) {
            applog(LOG_ERR, "FAIL: thread %u failed to initialize", i);
            exit(1);
        }
    }
